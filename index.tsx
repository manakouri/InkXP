// Fix: Import React, ReactDOM, and necessary hooks.
import React, { useState, useEffect, useCallback, useMemo, useRef } from 'react';
import ReactDOM from 'react-dom/client';

// Fix: Declare global variables for libraries loaded via script tags
...
// Fix: Define types for game data to ensure type safety
...
interface ScoreAnalysis {
    totalScore: number;
    breakdown: Array<{ id: string; name: string; count: number; score: number; }>;
...
// This is a helper function to call our Netlify serverless function
const callApi = async (task, payload) => {
    const response = await fetch('/api/gemini', { // Using the clean URL from netlify.toml
        method: 'POST',
        headers: {
...
const generateImageForPrompt = async (promptText) => {
    try {
        ...
        // Return null to allow the game to continue without an image
        return null;
    }
};
...
const getSentenceFeedback = async (rootSentence, userSentence) => {
    try {
        ...
        // Provide a default response on error so the game can continue
    ...
    // Always included goals
    {
        ...
        check: (text) => {
            ...
            const dialoguePatterns = [
                /"[^"]*," \w+ (said|asked|yelled|whispered|exclaimed|snapped|replied|muttered)\./i, // "...," she said.
                /\w+ (said|asked|yelled|whispered|exclaimed|snapped|replied|muttered), "[^"]*[\.?!]"/i, // He asked, "..."?
                /"[^"]*," \w+ (said|asked|yelled|whispered|exclaimed|snapped|replied|muttered), "[^"]*[\.?!]"/i, // "...," he said, "..."
                /"[^"]+[\.?!]"/ // "Are you coming?"
            ];
...
const downloadPDF = (nickname, story, prompt) => {
    // Fix: Use window.jspdf to access the library
...
// Fix: Add explicit prop types for Button component
...
// Fix: Add explicit prop types for Tooltip component
...
const ResultsScreen = ({ gameData, playerId, onPlayAgain, onGoHome }) => {
  ...
  // Fix: Add type casting to resolve property access errors
  const sortedPlayers = Object.entries(gameData.players)
    .map(([id, data]) => ({ id, ...(data as PlayerData), totalScore: (data as PlayerData).analysis?.totalScore || 0 }))
                    ...
                    <div className="flex items-start gap-4">
                        {gameData.promptImageUrl ? (
                             <img 
                                ...
                                className="w-24 h-24 object-cover rounded-lg shadow-sm border border-slate-200 flex-shrink-0"
                            />
                        ) : (
...
// components/PlayerDetailModal.js
              ...
              <ul className="space-y-2">
                  {playerList.map(([id, player]) => (
                      <li key={id} className="flex items-center gap-3 p-2 bg-slate-100 rounded-md">
                         {/* Fix: Add type casting to resolve property access errors */}
                         <span className="font-bold text-slate-700">{(player as PlayerData).nickname}</span>
...
// components/PlayerHistoryModal.js
...
// Fix: Define props type for TabButton
...
const TeacherDashboard = ({ currentUser, onCreateNewGame, onViewGame, onSignOut }) => {
  ...
  const [activeTab, setActiveTab] = useState('games'); // 'games' or 'players'
  ...
  useEffect(() => {
      ...
      const unsubscribe = getGamesForTeacher(currentUser.uid, (fetchedGames) => {
          ...
          const aggregatedPlayers = {}; // Key will be the individual, trimmed nickname
          fetchedGames.forEach(game => {
              // Fix: Add type casting to resolve property access errors
              Object.entries(game.players).forEach(([playerId, playerData]) => {
                  const pData = playerData as PlayerData;
                  if (pData.analysis && pData.nickname) {
                      const nicknames = pData.nickname.split(',').map(name => name.trim()).filter(Boolean);

                      nicknames.forEach(individualNickname => {
                          if (!aggregatedPlayers[individualNickname]) {
                              aggregatedPlayers[individualNickname] = {
                                  id: individualNickname, // Use nickname as a unique ID for aggregation
                                  nickname: individualNickname,
                                  writings: []
                              };
                          }
                          
                          // Push the writing, ensuring the full list of authors is preserved for attribution
                          aggregatedPlayers[individualNickname].writings.push({
                              gameId: game.id,
                              prompt: game.prompt,
                              createdAt: game.createdAt,
                              story: pData.story || { title: '', text: '' },
                              analysis: pData.analysis,
                              // Store the original full nickname string for attribution
                              authors: pData.nickname 
                          });
                      });
                  }
              });
          ...
          // Sort each player's writings by date
          Object.values(aggregatedPlayers).forEach((p: any) => {
  ...
  const sortedPlayers = useMemo(() => {
      ...
      // Fix: Add type casting to resolve property access errors
      return Object.values(playersData).sort((a: any, b: any) => a.nickname.localeCompare(b.nickname));
  }, [playersData]);
...
// components/SentenceBuilderScreen.js
const SentenceBuilderScreen = ({ onGoHome }) => {
    const [stage, setStage] = useState('rewrite'); // 'rewrite', 'kete', 'scoring', 'results'
    ...
    const calculateMechanicalScore = (baseSentence, newSentence, baseVerb) => {
        ...
        score += countWords(addedWords.join(' '), Object.values(KETE_WORDS).flat()) * 5; // General reward for kete words
        
        // Check if verb was changed
            ...
            <div className="text-center">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    <div className="bg-white p-4 rounded-lg shadow-md border border-slate-200">
                        <p className="text-sm font-bold text-slate-500">MECHANICAL SCORE</p>
                        <p className="text-3xl font-bold text-slate-700">{score.mechanicalScore}</p>
                    </div>
                    <div className="bg-white p-4 rounded-lg shadow-md border border-slate-200">
                        <p className="text-sm font-bold text-slate-500">AI QUALITY MULTIPLIER</p>
                        <p className="text-3xl font-bold text-violet-600">x{score.qualityScore.toFixed(2)}</p>
                    </div>
                ...
                <div className="flex items-center justify-center gap-4">
                    <Button onClick={onGoHome} variant="secondary" size="large">Back to Home</Button>
                    <Button onClick={handleTryAgain} size="large">Try Another One!</Button>
                </div>
            </div>
        );
    }
